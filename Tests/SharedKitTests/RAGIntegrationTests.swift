...
final class RAGIntegrationTests: XCTestCase {
    ...
    func testToolExecutionWithFeedback() async throws {
        ...
        XCTAssertTrue(
            response.lowercased().contains("status") ||
            response.lowercased().contains("running") ||
    ...
    func testEmbeddingPerformance() throws {
        measure {
            ...
            Task {
                _ = try await embeddingService.embed("This is a test query for performance measurement")
                exp.fulfill()
            }
