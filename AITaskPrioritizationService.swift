...
public class AITaskPrioritizationService {
    ...
    private func generateTimeBasedSuggestions(tasks: [TaskItem]) async -> [TaskSuggestion] {
        ...
        let tasksByCategory = Dictionary(
            grouping: tasks.filter { !$0.isCompleted }, by: { $0.category }
        )
        ...
        await withCheckedContinuation { continuation in
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                Task {
                    ...
                    let sortedInsights = insights.sorted {
    ...
    private func analyzeProductivityPatterns(_ activities: [ActivityRecord]) async
        -> [ProductivityInsight]
    {
        var insights: [ProductivityInsight] = []
        ...
        let scoreInsight = ProductivityInsight(
            ...
            description: String(
                format: "%.1f/10 - Based on task completion (%.0f%%) and focus time (%.1f hours)",
    ...
    private func analyzeTaskPatterns(_ tasks: [TaskItem]) async -> [ProductivityInsight] {
        ...
        if !completedTasks.isEmpty {
            ...
            let completionInsight = ProductivityInsight(
                ...
                description: String(
                    format: "Average time to complete tasks: %.1f hours", averageCompletionTime
                ),
                icon: "speedometer",
                priority: .medium,
                category: .efficiency,
    ...
    private func analyzeGoalProgress(_ goals: [GoalItem], _ tasks: [TaskItem]) async
        -> [ProductivityInsight]
    {
        var insights: [ProductivityInsight] = []
    ...
    private func calculateCompletionRate(_ activities: [ActivityRecord]) -> Double {
        let taskActivities = activities.filter {
            $0.type == .taskCompleted || $0.type == .taskCreated
        }
