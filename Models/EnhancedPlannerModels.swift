...
public final class EnhancedTask: Validatable, Trackable, CrossProjectRelatable {
    ...
    public var urgencyScore: Int {
        ...
        if self.isOverdue {
            score += 50
        } else if self.isDueToday {
            score += 30
        } else if self.isDueTomorrow {
            score += 20
        } else if self.isDueThisWeek {
            score += 10
        }

        if self.energyLevel == .low { score -= 5 } else if self.energyLevel == .high { score += 5 }
    ...
    public static func from(ckRecord: CKRecord) throws -> EnhancedTask {
        ...
        let task = EnhancedTask(
            ...
            category: TaskCategory(rawValue: ckRecord["category"] as? String ?? "general")
                ?? .general
        )
...
public final class EnhancedGoal: Validatable, Trackable, CrossProjectRelatable {
    ...
    public var daysRemaining: Int {
        ...
        return max(
            0, Calendar.current.dateComponents([.day], from: Date(), to: targetDate).day ?? 0
        )
    }
    ...
    public func complete() {
        ...
        self.trackEvent(
            "goal_completed",
            parameters: [
                "days_to_complete": Calendar.current.dateComponents(
                    [.day], from: self.createdAt, to: Date()
                ).day ?? 0,
    ...
    private func updateAverageProgress() {
        let weeksSinceCreation = max(
            1,
            Calendar.current.dateComponents([.weekOfYear], from: self.createdAt, to: Date())
                .weekOfYear ?? 1
        )
...
public final class EnhancedJournalEntry: Validatable, Trackable, CrossProjectRelatable {
    ...
    public init(title: String, content: String, date: Date = Date()) {
        ...
        self.wordCount = content.components(separatedBy: .whitespacesAndNewlines).count(where: {
            !$0.isEmpty
        })
        self.readingTime = 0
    ...
    public func trackEvent(_ event: String, parameters: [String: Any]? = nil) {
        ...
        print(
            "Tracking event: \(event) for journal entry: \(self.title) with parameters: \(eventParameters)"
        )
    }
    ...
    public func updateContent(_ newContent: String) {
        ...
        self.wordCount = newContent.components(separatedBy: .whitespacesAndNewlines).count(where: {
            !$0.isEmpty
        })
        self.modifiedAt = Date()
