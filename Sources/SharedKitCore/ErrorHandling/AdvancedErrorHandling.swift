...
public protocol AppErrorProtocol: Error, LocalizedError, CustomStringConvertible, Codable {
    ...
    var context: [String: AnyCodable] { get }
...
public struct AppError: AppErrorProtocol, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        // Decode context
        if let contextData = try? container.decode([String: AnyCodable].self, forKey: .contextData) {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        // Encode context
        try container.encode(self.context, forKey: .contextData)
    }
...
public struct AppValidationError: AppErrorProtocol, Sendable {
    public let errorId: String
    ...
    public let providedValue: AnyCodable?
    ...
    public init(
        ...
        self.providedValue = providedValue.map { AnyCodable($0) }
        ...
        var contextDict: [String: AnyCodable] = [
            "field": .string(field),
            "validation_rule": .string(validationRule),
        ]
        if let value = providedValue {
            contextDict["provided_value"] = .string("\(value)")
        }
        if let format = expectedFormat {
            contextDict["expected_format"] = .string(format)
        }
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.providedValue = try? container.decode(AnyCodable.self, forKey: .providedValue)
...
public struct NetworkError: AppErrorProtocol, Sendable {
    ...
    public init(
        ...
        if let code = statusCode {
            contextDict["status_code"] = .int(code)
        }
        if let url = endpoint {
            contextDict["endpoint"] = .string(url)
        }
        if let method = httpMethod {
            contextDict["http_method"] = .string(method)
        }
        if let data = responseData {
            contextDict["response_size"] = .int(data.count)
        }
        self.context = contextDict
    }
...
public struct BusinessError: AppErrorProtocol, Sendable {
    ...
    public init(
        ...
        var contextDict: [String: AnyCodable] = [
            "business_rule": .string(businessRule),
            "violated_constraint": .string(violatedConstraint),
        ]
        if let entity = affectedEntity {
            contextDict["affected_entity"] = .string(entity)
        }
        self.context = contextDict
    }
...
public final class ErrorHandlerManager: ObservableObject {
    ...
    public func handleError(_ error: any AppErrorProtocol) async {
        self.logger.error("Error occurred: \(error.description, privacy: .public)")
        ...
        await self.analyticsService.track(
            ...
            properties: [
                ...
                "severity": .string(error.severity.rawValue),
    ...
    public func handleValidationError(
        ...
        let validationError = AppValidationError(
            field: field,
            validationRule: rule,
            providedValue: value.map { AnyCodable($0) },
    ...
    private func attemptRecovery(for error: any AppErrorProtocol) async -> RecoveryResult {
        ...
        await self.analyticsService.track(
            ...
            properties: [
                "error_id": .string(error.errorId),
                "category": .string(error.category.rawValue),
                "strategies_attempted": .int(strategies.count),
            ],
            userId: nil as String?
        )
...
public enum GlobalErrorState: String, Codable, Sendable {
    case normal
    case degraded
    case critical
    ...
    public func reportError(_ error: any AppErrorProtocol) async throws {
        await self.analyticsService.track(
            ...
            properties: [
                ...
                "category": .string(error.category.rawValue),
                "severity": .string(error.severity.rawValue),
                "user_message": .string(error.userMessage),
                "technical_details": .string(error.technicalDetails),
                "timestamp": .double(error.timestamp.timeIntervalSince1970),
                "context": .string(String(describing: error.context)),
            ],
            userId: nil as String?
        )
    }
...
public struct DebugErrorReporter: ErrorReporter {
    ...
    public func reportError(_ error: any AppErrorProtocol) async throws {
        self.logger.fault(
            "ERROR: [\(error.category.displayName, privacy: .public)] \(error.userMessage, privacy: .public) | Technical: \(error.technicalDetails, privacy: .public) | Context: \(String(describing: error.context), privacy: .public)"
        )
    }
}
