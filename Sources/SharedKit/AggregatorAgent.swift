...
public final class AggregatorAgent: Sendable {
    ...
    public func clearHistory() {
        conversationHistory = []
        print("🗑️ Agent: Conversation history cleared")
    }
    ...
    public func process(query: String) async throws -> String {
        print("🤖 Aggregator: Received query: '\(query)'")
        ...
        print("🔍 Aggregator: RAG Context found: \(context.count) items")
        ...
        print("📋 Aggregator: Plan created: \(cleanPlan)")
        ...
        if cleanPlan.contains("TOOL: status") || cleanPlan.contains("TOOL: health") {
            print("🛠️ Aggregator: Delegating to Tool: status")
            ...
            print("🛠️ Aggregator: Delegating to Tool: logs")
            ...
            print("🛠️ Aggregator: Delegating to Tool: build")
            ...
            print("🛠️ Aggregator: Delegating to Tool: deploy")
            ...
            print("🛠️ Aggregator: Delegating to Tool: metrics")
            ...
            print("🛠️ Aggregator: Delegating to Tool: backup")
            let output = try await runDockerManagerWithRetry(command: "exec", args: ["db", "pg_dump", "-U", "sonar", "sonar"])
            toolResult = ToolExecutionResult.parse(toolName: "backup", rawOutput: output)
        } else if cleanPlan.contains("TOOL: ai-fix") {
            print("🛠️ Aggregator: Delegating to Tool: ai-fix")
            let output = try await runDockerManagerWithRetry(command: "ai-fix", args: ["tools-automation"])
            toolResult = ToolExecutionResult.parse(toolName: "ai-fix", rawOutput: output)
        } else if cleanPlan.contains("TOOL: start") {
            print("🛠️ Aggregator: Delegating to Tool: start")
            let output = try await runDockerManagerWithRetry(command: "start", args: ["core"])
            toolResult = ToolExecutionResult.parse(toolName: "start", rawOutput: output)
        } else if cleanPlan.contains("TOOL: stop") {
            print("🛠️ Aggregator: Delegating to Tool: stop")
        ...
        if let result = toolResult {
            ...
            response = try await synthesizeResponse(query: query, context: contextString, toolOutput: feedbackContext)
        } else {
    ...
    private func synthesizeResponse(query: String, context: String, toolOutput: String) async throws -> String {
    ...
    private func runDockerManagerWithRetry(command: String, args: [String] = [], maxRetries: Int = 3) async throws -> String {
        var lastError: Error?
        ...
        for attempt in 0..<maxRetries {
            do {
                ...
                lastError = error
                print("⚠️ Attempt \(attempt + 1)/\(maxRetries) failed: \(error.localizedDescription)")
        ...
        throw lastError ?? NSError(domain: "AggregatorAgent", code: -1, userInfo: [NSLocalizedDescriptionKey: "All retries failed"])
    }
}
