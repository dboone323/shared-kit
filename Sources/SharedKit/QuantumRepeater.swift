...
public class QuantumRepeater: ObservableObject {
    ...
    public func performEntanglementSwapping(
        ...
        if swappingSuccess {
            ...
            print(
                "- Extended range from \(String(format: "%.1f", pair1.distance + pair2.distance)) km to \(String(format: "%.1f", newDistance)) km"
            )
    ...
    public func purifyEntanglement(_ pair: EntanglementPair) async -> EntanglementPair? {
        ...
        if purificationSuccess {
            ...
            print(
                "- Improved fidelity from \(String(format: "%.3f", pair.currentFidelity)) to \(String(format: "%.3f", purifiedFidelity))"
            )
    ...
    public func monitorPerformance() async -> RepeaterPerformanceMetrics {
        ...
        let averageFidelity = entanglementPairs.map(\.currentFidelity).reduce(0, +) / Double(max(
            1,
            entanglementPairs.count
        ))
