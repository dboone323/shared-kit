...
public actor FeedbackLearningSystem {
    ...
    public struct ResponseFeedback: Sendable, Codable {
        ...
        public init(responseId: UUID, query: String, response: String, rating: Rating, toolUsed: String? = nil) {
    ...
    public func getImprovementSuggestions() -> [String] {
        ...
        if stats.successRate < 0.7 && stats.totalFeedback > 10 {
            suggestions.append("Overall success rate is low (\(Int(stats.successRate * 100))%). Consider improving planning logic or adding more context.")
        }
        ...
        for (tool, perf) in stats.toolPerformance {
            ...
            if toolSuccess < 0.5 && perf.total >= 3 {
                suggestions.append("Tool '\(tool)' has low success rate (\(Int(toolSuccess * 100))%). Review its implementation.")
            }
        }
