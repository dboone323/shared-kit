...
public struct MLQuantumState: Sendable {
...
public enum MLQuantumGate: Sendable {
...
public struct MLQuantumCircuit: Sendable {
    public let numQubits: Int
    public let gates: [MLQuantumGate]

    public init(numQubits: Int, gates: [MLQuantumGate] = []) {
        self.numQubits = numQubits
        self.gates = gates
    }

    public func addingGate(_ gate: MLQuantumGate) -> MLQuantumCircuit {
        MLQuantumCircuit(numQubits: numQubits, gates: gates + [gate])
    }
}
...
public actor MLQAOAOptimizer {
    private let logger = os.Logger(subsystem: "com.quantum.workspace", category: "QAOAOptimizer")
    ...
    private func createQAOACircuit(numQubits: Int, layers: Int) -> MLQuantumCircuit {
        var circuit = MLQuantumCircuit(numQubits: numQubits)
...
public actor MLVariationalQuantumClassifier {
    private let logger = os.Logger(subsystem: "com.quantum.workspace", category: "VQClassifier")
...
public actor MLQuantumSVM {
    private let logger = os.Logger(subsystem: "com.quantum.workspace", category: "QuantumSVM")
    private var supportVectors: [([Double], Int)] = []
    private var alphas: [Double] = []
    private let kernel: MLQuantumKernel

    public init(kernel: MLQuantumKernel = .gaussian(sigma: 1.0)) {
        self.kernel = kernel
    }
...
public enum MLQuantumKernel: Sendable {
    case linear
...
public actor MLQuantumNeuralNetwork {
    private let logger = os.Logger(subsystem: "com.quantum.workspace", category: "QNN")
...
public actor MLQuantumMLRegistry {
    private let logger = os.Logger(subsystem: "com.quantum.workspace", category: "MLRegistry")
    private var classifiers: [String: MLVariationalQuantumClassifier] = [:]
    private var svms: [String: MLQuantumSVM] = [:]
    private var networks: [String: MLQuantumNeuralNetwork] = [:]

    public func registerClassifier(_ classifier: MLVariationalQuantumClassifier, withId id: String)
    {
        classifiers[id] = classifier
        logger.info("📝 Registered VQC: \(id)")
    }

    public func registerSVM(_ svm: MLQuantumSVM, withId id: String) {
        svms[id] = svm
        logger.info("📝 Registered Quantum SVM: \(id)")
    }

    public func registerNetwork(_ network: MLQuantumNeuralNetwork, withId id: String) {
        networks[id] = network
        logger.info("📝 Registered QNN: \(id)")
    }

    public func getClassifier(withId id: String) -> MLVariationalQuantumClassifier? {
        classifiers[id]
    }

    public func getSVM(withId id: String) -> MLQuantumSVM? {
        svms[id]
    }

    public func getNetwork(withId id: String) -> MLQuantumNeuralNetwork? {
        networks[id]
    }
...
public enum MLQuantumMLError: Error {
...
private let globalRegistry = MLQuantumMLRegistry()
...
public func createVariationalQuantumClassifier(id: String, numQubits: Int, layers: Int = 3) async {
    let classifier = MLVariationalQuantumClassifier(numQubits: numQubits, layers: layers)
    await globalRegistry.registerClassifier(classifier, withId: id)
}

/// Create and register a quantum SVM
@MainActor
public func createQuantumSVM(id: String, kernel: MLQuantumKernel = .gaussian(sigma: 1.0)) async {
    let svm = MLQuantumSVM(kernel: kernel)
...
public func createQuantumNeuralNetwork(
    ...
    let qnn = MLQuantumNeuralNetwork(
