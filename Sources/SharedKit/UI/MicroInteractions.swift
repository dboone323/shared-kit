            ...
            DragGesture()
                .onChanged { value in
                    ...
                    if direction != self.swipeDirection {
                        ...
                        withAnimation(AnimationTiming.quick) {
                            self.showDirectionHint =
                                abs(value.translation.width) > self
                                .threshold / 2
                                || abs(value.translation.height) > self
                                    .threshold / 2
                        }
                    }
                }
                .onEnded { value in
                    ...
                    if abs(translation.width) > self.threshold
                        || abs(translation.height) > self.threshold
                    {
...
public struct ContextMenuInteraction<Content: View, MenuContent: View>: View {
    ...
    public var body: some View {
        self.content
            ...
            .onLongPressGesture(
                minimumDuration: 0.2, maximumDistance: 50,
                pressing: { pressing in
                    withAnimation(AnimationTiming.quick) {
...
public struct FluidInteractionArea<Content: View>: View {
    let content: Content
    let onInteraction: (UIInteractionType, CGPoint) -> Void

    @State private var lastTouchPoint: CGPoint = .zero
    @State private var ripples: [RippleEffect] = []

    public init(
        onInteraction: @escaping (UIInteractionType, CGPoint) -> Void = { _, _ in },
        @ViewBuilder content: () -> Content
    ) {
        self.content = content()
        ...
        .gesture(
            DragGesture(minimumDistance: 0)
                .onChanged { value in
                    ...
                    self.onInteraction(UIInteractionType.drag, value.location)
                }
                .onEnded { value in
                    self.onInteraction(UIInteractionType.end, value.location)
                    self.createRipple(at: value.location)
                }
        )
        .onTapGesture { location in
            self.onInteraction(UIInteractionType.tap, location)
...
extension View {
    public func microInteractionButton(
        feedbackStyle: Int = 1,
        scaleEffect: CGFloat = 0.95,
        glowColor: Color? = nil,
        action: @escaping () -> Void
    ...
    public func pinchToZoom(
        ...
        modifier(
            PinchToZoomModifier(
                minScale: minScale, maxScale: maxScale, onScaleChange: onScaleChange))
    }

    public func rotationGesture(onRotationChange: @escaping (Angle) -> Void = { _ in }) -> some View
    {
        modifier(RotationGestureModifier(onRotationChange: onRotationChange))
    }

    public func multiTouchGestures(
        onSingleTap: (() -> Void)? = nil,
        onDoubleTap: (() -> Void)? = nil,
        onTripleTap: (() -> Void)? = nil,
        onLongPress: (() -> Void)? = nil
