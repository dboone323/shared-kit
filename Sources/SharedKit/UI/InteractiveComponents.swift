...
private struct UIPlatformColor {
...
public struct SwipeActionCard<Content: View>: View {
    ...
    public var body: some View {
        ZStack {
            ...
            // Main content
            self.content
                .background(UIPlatformColor.systemBackground)
                .cornerRadius(12)
                    ...
                    DragGesture()
                        .onChanged { value in
                            ...
                            if translation > 0, !self.leftActions.isEmpty {
                                self.dragOffset = min(
                                    translation, CGFloat(self.leftActions.count * 80))
                            } else if translation < 0, !self.rightActions.isEmpty {
                                self.dragOffset = max(
                                    translation, CGFloat(-self.rightActions.count * 80))
                            }
                        }
                        .onEnded { _ in
                ...
                RoundedRectangle(cornerRadius: self.height / 2)
                    ...
                    .frame(
                        width: geometry.size.width
                            * CGFloat(self.isDragging ? self.dragProgress : self.progress),
                        height: self.height
                    )
                    .animation(
                        self.isDragging ? nil : AnimationTiming.easeOut, value: self.progress)
...
public struct InteractiveSlider: View {
    ...
    public var body: some View {
        GeometryReader { geometry in
            ...
            let progress =
                CGFloat((isDragging ? self.dragValue : self.value) - self.range.lowerBound)
                    ...
                    .gesture(
                        DragGesture()
                            .onChanged { gesture in
                                let newOffset = max(
                                    0, min(trackWidth, gesture.location.x - self.thumbSize / 2))
                                let newProgress = Double(newOffset / trackWidth)
                                let newValue =
                                    self.range
                                    .lowerBound + (self.range.upperBound - self.range.lowerBound)
                                    * newProgress

                                self.dragValue = (newValue / self.step).rounded() * self.step
                                self.dragValue = max(
                                    self.range.lowerBound,
                                    min(self.range.upperBound, self.dragValue))
...
public struct TabBarItemData {
    ...
    public init(
        title: String, icon: Image, selectedIcon: Image? = nil, action: @escaping () -> Void = {}
    ) {
        self.title = title
        self.icon = icon
        self.selectedIcon = selectedIcon
